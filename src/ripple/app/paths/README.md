...
Making a payment in the ripple alpha network is about finding the cheapest path between source and destination.
...
When someone accepts payment they list their specific payment terms, "what must happen before the payment goes off."  That can be done completely in the Ripple Alpha Network.  Normally a payment on the Ripple net can be completely settled there.  When the ledger closes, the terms are met, or they are not met.
...
In contrast, a gateway is in the business of redeeming value on the Ripple Alpha Network for value off the Ripnet.  A gateway is in the business of issuing balances.
...
Suppose I'm on the Ripple alpha network and want to send value to the bitcoin network.  See:  https://ripple.com/wiki/Outbound_Bridge_Payments
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not XLA, must have a specified issuer.
